...
const Navbar: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch({ type: 'SET_ARTICLES', payload: results });
      dispatch({ type: 'SET_CATEGORY', payload: 'search' });
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-2">
              ...
              <form onSubmit={handleSearch} className="hidden sm:block">
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-10 w-64 h-10 bg-secondary/50 border-border/50 focus:bg-background rounded-xl"
                  />
                  {isSearching && (
              ...
              <button
                ...
                className="relative p-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-colors"
                title="Bookmarks"
              >
              ...
              {/* Mobile menu button */}
              <button
                className="p-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-colors lg:hidden"
              ...
              <form onSubmit={handleSearch} className="sm:hidden mb-4">
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) => setSearchQuery(e.target.value)}
                    disabled={isSearching}
                    className="pl-10 w-full h-10 bg-secondary/50 border-border/50 rounded-xl"
                  />
                  {isSearching && (
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                      <div className="animate-spin h-4 w-4 border-2 border-primary border-t-transparent rounded-full"></div>
                ...
                {categories.map((category) => {
                  ...
                  return (
                    <button
                      key={category.id}
                      onClick={() => handleCategoryChange(category.id)}
                      className={`flex items-center justify-start px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                        state.selectedCategory === category.id 
                          ? 'bg-primary text-primary-foreground shadow-md' 
                          : 'text-muted-foreground hover:text-foreground hover:bg-secondary/50'
                      }`}
              ...
              {state.bookmarks.length === 0 ? (
                ...
                <div className="space-y-3">
                  {state.bookmarks.map((article) => (
                    <div key={article.id} className="bg-secondary/30 rounded-xl p-3 cursor-pointer hover:bg-secondary/50 transition-colors">
